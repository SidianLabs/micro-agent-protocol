...
export function resolveServerOptionsFromEnv(
  ...
  return {
    ...
    deadLetterStorePath: env.MAP_DEAD_LETTER_STORE_PATH ?? ".map/dead-letters.json",
    ...
    asyncQueueMaxRetryDelayMs: Number(env.MAP_ASYNC_MAX_RETRY_DELAY_MS ?? 5_000),
    ...
    asyncQueueMaxConcurrentPerTenant: parseOptionalNumber(env.MAP_ASYNC_MAX_CONCURRENT_PER_TENANT),
    ...
    healthMaxOldestDeadLetterAgeMs: parseOptionalNumber(env.MAP_HEALTH_MAX_OLDEST_DL_AGE_MS),
    metricsFailureRateThreshold: parseOptionalNumber(env.MAP_METRICS_FAILURE_RATE_THRESHOLD),
    ...
    rateLimitMaxRequestsPerTenant: parseOptionalNumber(env.MAP_RATE_LIMIT_MAX_REQUESTS_PER_TENANT),
    auditStorePath: env.MAP_AUDIT_STORE_PATH ?? ".map/audit-events.json",
    alertStorePath: env.MAP_ALERT_STORE_PATH ?? ".map/alerts.json",
    runtimeControlStorePath: env.MAP_RUNTIME_CONTROL_STORE_PATH ?? ".map/runtime-controls.json",
    auditMaxEvents: parseOptionalNumber(env.MAP_AUDIT_MAX_EVENTS),
    auditCheckpointInterval: parseOptionalNumber(env.MAP_AUDIT_CHECKPOINT_INTERVAL),
    signingRetiringKeyCriticalRatio: parseOptionalNumber(env.MAP_SIGNING_RETIRING_KEY_CRITICAL_RATIO),
    signingUnknownKeyCriticalRatio: parseOptionalNumber(env.MAP_SIGNING_UNKNOWN_KEY_CRITICAL_RATIO),
    includeExampleAgents: env.MAP_INCLUDE_EXAMPLE_AGENTS === "true"
  };
}
