...
export class ReceiptStore {
  ...
  append(receipt: ExecutionReceipt): void {
    ...
    if (existing) {
      if (JSON.stringify(existing) !== JSON.stringify(receipt)) {
        throw new Error(`Receipt conflict for immutable receipt id: ${receipt.receipt_id}`);
      }
      return;
    }
  ...
  get(receiptId: string, tenantId?: string): ExecutionReceipt | undefined {
    ...
    if (tenantId && this.resolveTenantId(receipt.tenant_id) !== this.resolveTenantId(tenantId)) {
      return undefined;
    }
    return receipt;
  }
  ...
  private persist(): void {
    ...
    writeFileSync(this.filePath, JSON.stringify({ receipts: this.list() }, null, 2), "utf8");
  }
