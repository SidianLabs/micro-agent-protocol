...
-  **Partial** — Implemented but missing some features or tests
...
| `use` (middleware) |  |  |  | Middleware stack registration |
...
| `stableStringify` (sorted keys) |  |  |  | Go uses struct field order |
| Payload ordering: body, key_id, method, path, timestamp |  |  |  | Alphabetical / canonical |
| Nonce generation (UUID v4) |  |  |  | `randomUUID()` |
| Body hashing (HMAC-SHA256) |  |  |  | Go uses full signing, not separate hash |
...
| Middleware pipeline |  |  |  | Request/response/error hooks |
...
| HTTP/2 support |  |  |  | Depends on runtime |
...
| `DelegationToken` |  |  |  | Trust/delegation types |
...
| Feature | TypeScript SDK | Python SDK | Go SDK | Notes |
|---|---|---|---|---|
| In-memory task store |  |  |  | InMemoryStorage |
...
| Policy rules & conditions |  |  |  | PolicyRule, PolicyCondition |
| Policy effects (allow/deny/require_approval) |  |  |  | PolicyEffect |
...
| Feature | TypeScript SDK | Python SDK | Go SDK | Notes |
|---|---|---|---|---|
| Structured logging |  |  |  | MAPLogger, LogLevel |
